...
import { createPostTemplate, deletePostTemplate, selectPostsTemplate } from "./query-tamplates";
...
export const getPosts = (userId: string): Promise<Post[]> =>
  ...
  export const insertPost = (userId: string, title:string, body:string) =>
    new Promise((resolve, reject) => {
      connection.all(createPostTemplate, [generateUniqueId(), userId, title, body], (error, results) => {
        if (error) {
          reject(error);
        }
